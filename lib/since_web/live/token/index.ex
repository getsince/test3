defmodule SinceWeb.TokenLive.Index do
  ...
  def handle_event("remove-token", %{"token" => token, "context" => context}, socket) do
    ...
    raw_token = Since.Accounts.UserToken.raw_token(token)
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen w-full">
      ...
      <%= if @live_action == :show do %>
        <div
          id={"tokens-" <> @user_id}
          class="fixed top-0 left-0 w-full h-full overflow-auto opacity-100 phx-modal"
          ...
          <div class="mt-10 max-w-2xl mx-auto rounded border dark:border-gray-600 bg-white dark:bg-gray-800 overflow-hidden">
            <div class="flex items-center justify-between p-4 bg-gray-100 dark:bg-gray-700 border-b dark:border-gray-600">
              <div class="flex items-center space-x-4">
                ...
                <button
                  phx-click="add-token"
                  class="bg-green-300 dark:bg-green-900 border border-green-600 font-semibold px-2 leading-7 rounded hover:bg-green-400 dark:hover:bg-green-800 transition"
                >
                  Add token
                </button>
              </div>
  ...
  defp user_card(assigns) do
    ...
    <div class="p-2 w-1/2 sm:w-1/4 lg:w-1/6 xl:w-1/8">
      <%= live_patch id: "user-" <> @user.id, to: Routes.token_index_path(SinceWeb.Endpoint, :show, @user.id) do %>
        <div class="rounded border p-2 bg-gray-50 dark:bg-gray-800 dark:text-gray-600 text-gray-500 dark:border-gray-700 hover:bg-blue-100 dark:hover:bg-blue-800 hover:border-blue-500 dark:hover:border-blue-700 hover:text-blue-600 transition">
  ...
  defp token(%{token: %{id: token_id, token: raw_token}} = assigns) do
    assigns =
      assign(assigns,
        ...
        encoded_token: Since.Accounts.UserToken.encoded_token(raw_token)
      )

    ~H"""
    <div>
      <div class="space-x-1 text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 dark:border-gray-600 text-xs inline-block rounded-t border-t border-r border-l divide-x dark:divide-gray-600 select-none">
      ...
      <div class="flex space-x-2">
        <p
          id={"token-" <> @token.id}
          class="font-mono bg-gray-100 dark:bg-gray-700 dark:border-gray-600 rounded-b rounded-r border px-1 py-1 inline-block"
        >
          <%= @encoded_token %>
        </p>
        <button
          phx-click="remove-token"
          phx-value-token={@encoded_token}
          phx-value-context={@token.context}
          class="bg-red-200 dark:bg-red-900 border px-2 border-red-500 dark:border-red-700 rounded hover:bg-red-300 dark:hover:bg-red-700 transition"
        >
          Delete
        </button>
      </div>
...
defmodule SinceWeb.TokenLive.Index.Ctx do
  import Ecto.Query
  alias Since.{Repo, Accounts.Profile, Accounts.UserToken}
  ...
  def remove_token(token, context) do
    SinceWeb.UserAuth.log_out_mobile_user(token, context)
  end
end
