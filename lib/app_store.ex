defmodule AppStore do
  ...
  @type env :: :dev | :prod
  @type notification :: %{topic: String.t(), env: env}
  ...
  @doc "Pushes a test notification to APNs"
  ...
  defp build_request(%{topic: topic, env: env} = notification) do
    token = AppStore.Token.current_token(topic, env)
  ...
  defp build_request(notification, token) do
    %{topic: topic, env: env} = notification
