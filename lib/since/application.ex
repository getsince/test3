defmodule Since.Application do
  ...
  defp maybe_oban do
    if repo_url() do
      if Since.Cluster.is_primary() do
        {Oban, oban_config()}
      else
  ...
  defp maybe_events do
    if config = Application.get_env(:since, Since.Events) do
      {Since.Events, config}
    else
  ...
  defp maybe_migrator do
    if Application.get_env(:since, :run_migrations_on_start?) && Since.Cluster.is_primary() do
