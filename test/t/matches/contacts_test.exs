defmodule T.Matches.ContactsTest do
  ...
  describe "save_contact_offer/3 for invalid match" do
    ...
    test "with non-existent match", %{profiles: [p1, _]} do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        Matches.save_contact_offer_for_match(match, p1.user_id, @contact)
      end
    end
    ...
    test "with match we are not part of", %{profiles: [p1, p2]} do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        Matches.save_contact_offer_for_match(match.id, p3.user_id, @contact)
      end
  ...
  describe "save_contact_offer/3" do
    ...
    test "with empty contact", %{profiles: [p1, _], match: match} do
      assert {:error, %Ecto.Changeset{valid?: false} = changeset} =
               Matches.save_contact_offer_for_match(match.id, p1.user_id, %{
                 "contact_type" => "",
                 "value" => ""
               })
  ...
  describe "save_slots_offer/3 side-effects" do
    setup [:with_profiles]
