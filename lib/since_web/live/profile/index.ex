defmodule SinceWeb.ProfileLive.Index do
  ...
  def render(assigns) do
    ...
    <%= if @live_action == :sort_by_registration do %>
      <div
        ...
        class="p-4 space-y-4"
        phx-update="append"
        phx-hook="RegisteredProfilesInfiniteScroll"
  ...
  defp background_image(%{"background" => %{"s3_key" => s3_key}}) do
    %{s3_key: s3_key, url: Since.Media.user_imgproxy_cdn_url(s3_key, 500, force_width: true)}
  end
  ...
  def profile(assigns) do
    ...
    <div
      ...
      data-cursor-last-active={@profile.last_active}
      data-cursor-inserted-at={@profile.inserted_at}
  ...
  defp story_label(%{label: label, size: [size_width, _size_height]} = assigns) do
    ...
    url =
      if answer = label["answer"] do
        Since.Media.known_sticker_url(answer)
      end || label["url"]
...
defmodule SinceWeb.ProfileLive.Index.Ctx do
  @moduledoc false
  import Ecto.Query
  alias Since.{Repo, Accounts}
  alias Since.Accounts.{Profile, User}
